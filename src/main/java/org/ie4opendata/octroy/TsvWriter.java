...
public class TsvWriter extends JCasAnnotator_ImplBase {
	...
	public static AnalysisEngineDescription getDescription(File outputDirectory)
		...
		return AnalysisEngineFactory.createEngineDescription(TsvWriter.class, TsvWriter.PARAM_OUTPUT_FILE,
				outputDirectory);
	}
	...
	public void initialize(UimaContext context) throws ResourceInitializationException {
		...
		try {
			...
			pw.println("Document\tAmount\tAmount Start\tAmount End\t"
					+ "Company\tCompany Start\tCompany End\tNEQ\tOfficial Name\t" + "Reason\tReason Start\tReason End");
			pw.close();
		} catch (IOException e) {
	...
	public void process(JCas jCas) throws AnalysisEngineProcessException {
		try {
			PrintWriter pw = new PrintWriter(new FileWriter(outputFile, true)); // slow,
																				// but
																				// effective
	...
	private void printAnnotation(PrintWriter pw, Annotation ann) throws IOException {
		if (ann == null)
			...
			if (ann instanceof Company) {
				Company cmp = (Company) ann;
				String officialName = cmp.getOfficialName();
				String registrationNumber = cmp.getRegistrationNumber();
				if (officialName == null)
					officialName = "";
				if (registrationNumber == null)
					registrationNumber = "";
				pw.print("\t" + registrationNumber + "\t" + officialName);
			}
		}
	}
}
